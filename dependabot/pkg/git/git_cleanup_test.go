...
var _ = Describe("GitCloner cleanup", func() {
	...
	DescribeTable("cleanup temp directory based on global switch", func(tc testCase) {
		...
		if source.WithTempDir {
			dir, err := os.MkdirTemp("", "git-cleanup-test-")
		...
		if golden.ExpectTempDirEmpty {
			Expect(cloner.tempDir).To(BeEmpty())
		} else {
			Expect(cloner.tempDir).To(Equal(tempDir))
		}
	},
		Entry("cleanup enabled", testCase{
			description: "should cleanup temp directory when global switch is enabled",
			sourceFile:  "./testdata/git_cleanup/cleanup_enabled/source.yaml",
			goldenFile:  "./testdata/git_cleanup/cleanup_enabled/golden.yaml",
		}),
		Entry("cleanup disabled", testCase{
			description: "should keep temp directory when global switch is disabled",
			sourceFile:  "./testdata/git_cleanup/cleanup_disabled/source.yaml",
			goldenFile:  "./testdata/git_cleanup/cleanup_disabled/golden.yaml",
		}),
		Entry("empty temp directory", testCase{
			description: "should return nil for empty temp directory",
			sourceFile:  "./testdata/git_cleanup/empty_temp_dir/source.yaml",
			goldenFile:  "./testdata/git_cleanup/empty_temp_dir/golden.yaml",
		}),
	)
})
