...
func TestWithCustomField(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "CVE fields",
			fields: map[string]interface{}{
				CustomFieldCVEID:    "CVE-2023-1234",
				CustomFieldCVEScore: 7.5,
			},
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			issue := newIssue(WithCustomField(tc.fields))

			for key, value := range tc.fields {
				fieldValue, exist := issue.Fields.Unknowns.Value(key)
				g.Expect(exist).To(BeTrue())
				g.Expect(fieldValue).To(Equal(value))
			}
		})
	}
}
...
func TestMultipleOptions(t *testing.T) {
	...
	issue := newIssue(
		WithProject("TEST"),
		WithType(IssueTypeVulnerability),
		WithSummary("Test Vulnerability"),
		WithDescription("This is a test vulnerability"),
		WithAssignee("johndoe"),
		WithPriority("High"),
		WithLabels("security", "test"),
		WithAffectsVersion("1.0.0"),
		WithCustomField(map[string]interface{}{
			CustomFieldCVEID:    "CVE-2023-1234",
			CustomFieldCVEScore: 7.5,
		}),
	)

	g.Expect(issue.Fields.Project.Key).To(Equal("TEST"))
	g.Expect(issue.Fields.Type.Name).To(Equal(IssueTypeVulnerability))
	g.Expect(issue.Fields.Summary).To(Equal("Test Vulnerability"))
	g.Expect(issue.Fields.Description).To(Equal("This is a test vulnerability"))
	g.Expect(issue.Fields.Assignee.Name).To(Equal("johndoe"))
	g.Expect(issue.Fields.Priority.Name).To(Equal("High"))
	g.Expect(issue.Fields.Labels).To(ConsistOf("security", "test"))
	g.Expect(issue.Fields.AffectsVersions).To(HaveLen(1))
	g.Expect(issue.Fields.AffectsVersions[0].Name).To(Equal("1.0.0"))

	cveID, exist := issue.Fields.Unknowns.Value(CustomFieldCVEID)
	g.Expect(exist).To(BeTrue())
	g.Expect(cveID).To(Equal("CVE-2023-1234"))

	cveScore, exist := issue.Fields.Unknowns.Value(CustomFieldCVEScore)
	g.Expect(exist).To(BeTrue())
	g.Expect(cveScore).To(Equal(7.5))
}
