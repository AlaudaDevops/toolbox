...
func TestClient_LinkIssue(t *testing.T) {
	g := NewWithT(t)
	...
	tests := []struct {
		...
		parentID       string
		childID        string
		childFields    *jira.IssueFields
		mockLinkResp   string
		mockLinkStatus int
		wantErr        bool
	}{
		{
			name:     "link issues successfully",
			parentID: "10000",
			childID:  "10001",
			childFields: &jira.IssueFields{
				IssueLinks: []*jira.IssueLink{},
			},
			mockLinkStatus: http.StatusCreated,
			wantErr:        false,
		},
		{
		...
		},
		{
			...
			childID:        "10001",
			childFields:    &jira.IssueFields{},
			mockLinkResp:   `{"errorMessages":["Error linking issues"],"errors":{}}`,
			mockLinkStatus: http.StatusBadRequest,
			wantErr:        true,
		},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.Method == http.MethodPost && r.URL.Path == "/rest/api/2/issueLink" {
					w.WriteHeader(tt.mockLinkStatus)
					fmt.Fprintln(w, tt.mockLinkResp)
					return
				}
			}))
			defer ts.Close()
			...
			parent := &jira.Issue{ID: tt.parentID}
			child := &jira.Issue{ID: tt.childID, Fields: tt.childFields}

			err = client.LinkIssue(context.Background(), parent, child)
...
func TestClient_CompleteIssue(t *testing.T) {
	g := NewWithT(t)
	...
	tests := []struct {
		...
		mockSearchResp      string
		mockSearchStatus    int
		mockTransitionsResp string
		mockTransStatus     int
		mockDoTransResp     string
		mockDoTransStatus   int
		wantErr             bool
	}{
		{
		...
		{
			name:             "complete issue successfully",
			jql:              jql.NewJQL("TEST").FilterBy("summary", "~ 'Test Issue'"),
			mockSearchResp:   `{"issues":[{"id":"10000","key":"TEST-1","fields":{"status":{"statusCategory":{"key":"new"}},"issuelinks":[]}}]}`,
			mockSearchStatus: http.StatusOK,
			mockTransitionsResp: `{"transitions":[
                {"id":"11","to":{"statusCategory":{"key":"indeterminate"}}},
                {"id":"21","to":{"statusCategory":{"key":"done"}}}
            ]}`,
			mockTransStatus:   http.StatusOK,
			mockDoTransStatus: http.StatusNoContent,
			wantErr:           false,
		},
		{
			name:             "issue already completed",
			jql:              jql.NewJQL("TEST").FilterBy("summary", "~ 'Test Issue'"),
			mockSearchResp:   `{"issues":[{"id":"10000","key":"TEST-1","fields":{"status":{"statusCategory":{"key":"done"}},"issuelinks":[]}}]}`,
			mockSearchStatus: http.StatusOK,
			wantErr:          false,
		},
		{
		...
		},
		{
			...
			mockSearchResp:      `{"issues":[{"id":"10000","key":"TEST-1","fields":{"status":{"statusCategory":{"key":"new"}},"issuelinks":[]}}]}`,
			mockSearchStatus:    http.StatusOK,
			mockTransitionsResp: `{"errorMessages":["Error getting transitions"],"errors":{}}`,
			mockTransStatus:     http.StatusBadRequest,
			wantErr:             true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.Method == http.MethodGet && r.URL.Path == "/rest/api/2/issue/10000/transitions" {
					w.WriteHeader(tt.mockTransStatus)
					fmt.Fprintln(w, tt.mockTransitionsResp)
					return
				}
				if r.Method == http.MethodPost && r.URL.Path == "/rest/api/2/issue/10000/transitions" {
					w.WriteHeader(tt.mockDoTransStatus)
					fmt.Fprintln(w, tt.mockDoTransResp)
					return
				}
			}))
			defer ts.Close()
			...
			err = client.CompleteIssue(context.Background(), tt.jql)
...
func TestClient_GetComponentVersion(t *testing.T) {
	...
	tests := []struct {
		...
		projectKey      string
		component       string
		mockProjectResp string
		mockProjStatus  int
		expectedVersion string
		wantErr         bool
	}{
		{
			name:       "found component version",
			projectKey: "TEST",
			component:  "component-a",
			mockProjectResp: `{
                "key": "TEST",
                "versions": [
                    {"name": "component-a-1.0", "released": false},
                    {"name": "component-b-1.0", "released": false}
                ]
            }`,
			mockProjStatus:  http.StatusOK,
			expectedVersion: "component-a-1.0",
			wantErr:         false,
		},
		{
		...
		},
		{
			...
			projectKey:      "TEST",
			component:       "component-a",
			mockProjectResp: `{"errorMessages":["Project not found"],"errors":{}}`,
			mockProjStatus:  http.StatusNotFound,
			wantErr:         true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Setup test server
			ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.Method == http.MethodGet && r.URL.Path == "/rest/api/2/project/TEST" {
					w.WriteHeader(tt.mockProjStatus)
					fmt.Fprintln(w, tt.mockProjectResp)
					return
				}
			}))
			defer ts.Close()
			...
			version, err := client.GetComponentVersion(context.Background(), tt.projectKey, tt.component)
